...
/**
 * API Key is required if the message
 * should be imported (uses a different endpoint).
 *
 * @return {Boolean}
 */

Mixpanel.option('apiKey', function(msg, settings){
  if ('track' != msg.type()) return;
  if (!shouldImport(msg)) return;
  if (settings.apiKey) return;

  // TODO: make this easier, only a string is required?
  // also someone might forget to pass .settings and .message to the error
  return this.error('validation', '.apiKey is required for messages older than 5 days', {
    settings: settings,
    message: msg
  });
});
